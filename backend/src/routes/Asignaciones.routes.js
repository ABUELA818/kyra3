...
router.post("/:ID_Asignacion/enviar", upload.array("files"), async (req, res) => {
  const { ID_Asignacion } = req.params
  ...
  try {
    ...
    // 1. Obtener el estado actual de la asignación
    const estadoAnteriorResult = await client.query(
      'SELECT "Estado_Asignacion" FROM "Asignaciones" WHERE "ID_Asignacion" = $1',
      [ID_Asignacion],
    )
    ...
    // 2. Actualizar el estado de la asignación a "Enviados"
    await client.query('UPDATE "Asignaciones" SET "Estado_Asignacion" = $1 WHERE "ID_Asignacion" = $2', [
      "Enviados",
      ID_Asignacion,
    ])
    ...
    await client.query(
      'INSERT INTO "Historial_Asignacion" ("ID_Asignacion", "ID_Usuario", "Estado_Anterior", "Estado_Nuevo") VALUES ($1, $2, $3, $4)',
      [ID_Asignacion, ID_Usuario, estado_anterior, "Enviados"],
    )

    // 4. Si hay archivos, insertarlos en las tablas Archivo y Adjuntos
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        const storageKey = path.join(uploadsDir, file.filename)
        ...
        const archivoResult = await client.query(
          ...
          [file.originalname, file.size, file.mimetype, ID_Usuario, storageKey, `/uploads/${file.filename}`],
        )

        const ID_Archivo = archivoResult.rows[0].ID_Archivo

        // Insertar en tabla Adjuntos
        await client.query(
          ...
          [ID_Asignacion, ID_Archivo, ID_Usuario, "submission"],
        )
      }
    }
    ...
    res.status(200).json({
      message: "Asignación enviada exitosamente con archivos.",
      ID_Asignacion,
...
router.get("/:ID_Asignacion/archivos", async (req, res) => {
  try {
    const { ID_Asignacion } = req.params
    ...
    console.error("Error al obtener archivos de la asignación:", error)
    res.status(500).json({ message: "Error interno del servidor." })
  }
})
...
router.post("/", upload.array("archivos"), async (req, res) => {
  ...
  if (!Titulo_Asignacion || !Prioridad || !Fecha_Entrega || !Creado_Por || !usuarios || usuarios.length === 0) {
    return res.status(400).json({
      message:
        "Faltan datos obligatorios: Título, Prioridad, Fecha de Entrega, Creador y al menos un usuario asignado.",
    })
  }
  ...
  try {
    ...
    for (const userId of usuarios) {
      const detalles = {
        titulo: Titulo_Asignacion,
        descripcion: Descripción_Asignacion,
      ...
      await client.query(
        `INSERT INTO "Notificaciones" ("ID_Usuario", "Tipo_Noti", "Detalles", "Visto") 
         VALUES ($1, $2, $3, $4)`,
        [userId, "asignacion", JSON.stringify(detalles), false],
      )
    }

    if (req.files && req.files.length > 0) {
      console.log(`[v0] Procesando ${req.files.length} archivo(s) para la asignación ${newAssignmentId}`)

      // Obtener el ID_Equipo del proyecto para vincular los archivos
      let idEquipo = null
      if (ID_Proyecto) {
        const queryEquipo = `SELECT "ID_Equipo" FROM "Proyectos" WHERE "ID_Proyecto" = $1`
        const { rows: equipoRows } = await client.query(queryEquipo, [ID_Proyecto])
        idEquipo = equipoRows[0]?.ID_Equipo || null
      }
      ...
      for (const file of req.files) {
        ...
        const queryArchivo = `
          INSERT INTO "Archivo" (
            ...
            "Tipo_Archivo", "StorageKey", "Ruta"
          ) VALUES ($1, $2, $3, $4, $5, $6, $7)
          RETURNING "ID_Archivo";
        `

        const { rows: archivoRows } = await client.query(queryArchivo, [
          idEquipo,
          Creado_Por,
          file.originalname,
          file.size,
        ...
        // Vincular el archivo con la asignación en la tabla Adjuntos
        const queryAdjunto = `
          INSERT INTO "Adjuntos" ("ID_Asignacion", "ID_Archivo", "Subido_Por", "Tipo")
          VALUES ($1, $2, $3, $4);
        `

        await client.query(queryAdjunto, [
          newAssignmentId,
          idArchivo,
          Creado_Por,
          "creation", // Tipo de adjunto para diferenciar de archivos enviados después
        ])

        console.log(`[v0] Archivo registrado: ${file.originalname} (ID: ${idArchivo})`)
      }
    }
...
router.patch("/:ID_Asignacion/usuarios", async (req, res) => {
  ...
  if (
    (!usuarios_a_anadir || usuarios_a_anadir.length === 0) &&
    (!usuarios_a_quitar || usuarios_a_quitar.length === 0)
  ) {
...
router.delete("/:ID_Asignacion", async (req, res) => {
  ...
  try {
    ...
    const rolQuery =
      'SELECT r."Rol" FROM "Usuarios" u JOIN "Roles" r ON u."ID_Rol" = r."ID_Rol" WHERE u."ID_Usuario" = $1'
    ...
    if (creadorId !== ID_Usuario_solicitante && rolUsuario !== "Admin") {
      return res
        .status(403)
        .json({ message: "Permiso denegado. Solo el creador o un administrador pueden eliminar la asignación." })
    }
...
router.put("/:ID_Asignacion", async (req, res) => {
  const { ID_Asignacion } = req.params
  const { Titulo_Asignacion, Descripción_Asignacion, Prioridad, Estado_Asignacion, Fecha_Entrega, ID_Proyecto } =
    req.body

  if (!Titulo_Asignacion || !Prioridad || !Estado_Asignacion) {
    return res.status(400).json({
      message: "Se requieren título, prioridad y estado.",
    })
  }
