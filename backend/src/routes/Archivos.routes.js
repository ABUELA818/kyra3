...
//Subir/Crear Archivo
router.post("/upload", upload.array("archivos"), async (req, res) => {
  try {
    // 1. Extraer datos del cuerpo del formulario
...
// Get endpoint to retrieve file details by ID
router.get("/:ID_Archivo", async (req, res) => {
  try {
    const { ID_Archivo } = req.params

    if (!ID_Archivo) {
      return res.status(400).json({ message: "ID_Archivo es requerido." })
    }
...
router.delete("/:ID_Archivo", async (req, res) => {
  try {
    ...
    // Primero obtener el archivo para saber dónde está guardado
    ...
    // Eliminar favoritos asociados al archivo
    const deleteFavoritosQuery = `DELETE FROM "Favoritos" WHERE "ID_Archivo" = $1`
    await pool.query(deleteFavoritosQuery, [ID_Archivo])

    // Eliminar el archivo de la base de datos
    const deleteQuery = `DELETE FROM "Archivo" WHERE "ID_Archivo" = $1`
    await pool.query(deleteQuery, [ID_Archivo])

    // Aquí podrías eliminar el archivo físico del servidor si lo deseas
...
router.get("/:ID_Archivo/download", async (req, res) => {
  try {
    const { ID_Archivo } = req.params
